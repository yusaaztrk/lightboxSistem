...
const AdminSettings: React.FC = () => {
    ...
    const handleCreateComp = (type: 'profile' | 'backing' | 'adapter') => {
        ...
        if (type === 'profile') {
            ...
            setEditingItem({ id: 0, materialType: '', displayName: '', pricePerM2: 0 } as BackingCost);
    ...
    return (
        <div className="p-8 text-gray-200 pb-20 bg-[#0a0a0c] min-h-full rounded-3xl relative">
            <h1 className="text-3xl font-black text-white mb-2 uppercase tracking-tighter italic">Sistem Ayarları</h1>
            ...
            <div className="flex gap-4 border-b border-white/10 mb-8 overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('profileColors')} className={`pb-4 px-4 font-bold text-xs uppercase tracking-widest transition-colors ${activeTab === 'profileColors' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-500'}`}>PROFİL RENKLERİ</button>
                <button onClick={() => setActiveTab('wheel')} className={`pb-4 px-4 font-bold text-xs uppercase tracking-widest transition-colors ${activeTab === 'wheel' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-500'}`}>ÇARK AYARLARI</button>
                <button onClick={() => setActiveTab('leads')} className={`pb-4 px-4 font-bold text-xs uppercase tracking-widest transition-colors ${activeTab === 'leads' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-500'}`}>ÇEKİLİŞ VERİLERİ</button>
                <button onClick={() => setActiveTab('members')} className={`pb-4 px-4 font-bold text-xs uppercase tracking-widest transition-colors ${activeTab === 'members' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-500'}`}>ÜYELER</button>
                <button onClick={() => setActiveTab('membershipTypes')} className={`pb-4 px-4 font-bold text-xs uppercase tracking-widest transition-colors ${activeTab === 'membershipTypes' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-500'}`}>ÜYELİK TİPLERİ</button>
            </div>
